...
int encryptData(char *data, int dataLength)
{
	...
	NUM_rounds:                     // hopcount loop begin
		...
		lea	esi, gPasswordHash		// put ADDRESS of gPasswordHash into esi
			...
			mov ecx, gCurrentRounds // moves the current round count to ecx register which is used as a incrementor to get the correct byte values from the gPasswordHash
									// gCurrentRounds is a global variable
									//hopcount
		...
		exit_encrypt :
			...
			xor ecx, ecx;                  // "CLEAR REG ECX" for count & index
		...
		xor edx, edx						// "CLEAR REG EDX" for storage
			...
			shr al, 2                       // now shifts the data right by 2
	...
	ROTATE_ONE_BIT:						// Rotate one bit - beginning of loop
		xor eax, eax					// "CLEAR REG EAX" for storage
			mov al, byte ptr[edi + ecx]	// moves data from data array to al
			ror al, 1					// rotates the bits of al registers data to the right 1 time
		...
		xor eax, eax					// "CLEAR REG EAX" for storage	
			...
			rol al, 4					// rotates the al registers bits 4 times to the left				
			and dl, 0x0F				// logic AND of the lower edx register to clear the bits for rotation
			ror dl, 4					// rotates the dl registers bits 4 times to the right
		...
		mov ecx, gCurrentRounds         // restores the current round back into ecx (outer loop counter & round)
		inc ecx                         // increments the outer loop counter
		mov gCurrentRounds, ecx         // moves the count back into the round
		cmp ecx, gNumRounds             // compares the round provided by the user to the current loop round and if equal exits the outer loop of the hop count
		jne NUM_rounds                  
	}
